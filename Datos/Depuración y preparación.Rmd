...
```{r}
datos <- read.csv('CRON2W4e01.csv', header=TRUE)
View(datos)
nrow(datos)
...
-Por un lado, cogeremos las variables geográficas, demográficas, económicas. Además incluiremos por si acaso la variable de peso muestral (c2weight) por cuestiones técnicas.
-Por otro lado, cogeremos aquella variable psicológica tipo Likert relacionadas con felicidad.

Sin embargo, sobre las variables psicológicas, el interés principal del estudio girará en torno al ítem w4q50 concretamente, que alude directamente a una pregunta sobre felicidad ("Durante la semana pasada, ¿con qué frecuencia te sentiste feliz?"). 
```{r}
#Variables sociales
...
#COMPROBACIÓN DE VALORES NA, Y EN CASO AFIRMATIVO, TRANSFORMARLOS
```{r}

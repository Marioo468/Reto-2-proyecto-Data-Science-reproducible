...
internet <- ggplot(estudio2, aes(x = netusoft, y = w4q50, weight = c2weight)) +
...
ingreso <- ggplot(estudio2, aes(x = hincfel_inv, y = w4q50, weight = c2weight)) +
  geom_jitter(width = 0.15, height = 0.10, alpha = 0.25) +
  geom_smooth(method = "lm", formula = y ~ x, aes(weight = c2weight), se = FALSE, color = "blue") +
  facet_wrap(~cntry_name) +
...
```{r}
selectInput(
  "x_rel", "Relación (eje X) para dispersión:",
  choices = c("Uso de internet" = "netusoft",
              "Percepción de ingresos" = "hincfel",
              "Nivel educativo" = "eisced"),
  selected = "netusoft"
)

output$scatter_sel <- renderPlotly({
  xvar <- req(input$x_rel)  # "netusoft" | "hincfel" | "eisced"

  # Elegimos la variable factor/ordinal adecuada para etiquetas
  x_label <- case_when(
    xvar == "netusoft" ~ "Uso de internet",
    xvar == "hincfel"  ~ "Percepción de ingresos",
    xvar == "eisced"   ~ "Nivel educativo",
    TRUE ~ xvar
  )

  d <- estudio %>%
    filter(!is.na(.data[[xvar]]), !is.na(w4q50), !is.na(c2weight)) %>%
    mutate(x = as.numeric(.data[[xvar]]),
           y = as.numeric(w4q50))

  p <- ggplot(d, aes(x = x, y = y, weight = c2weight)) +
    geom_jitter(width = 0.15, height = 0.10, alpha = 0.25) +
    geom_smooth(method = "lm", formula = y ~ x,
                aes(weight = c2weight), se = FALSE, color = brand_dark) +
    labs(x = x_label, y = "Felicidad",
         title = paste0("Relación ponderada: ", x_label, " vs Felicidad")) +
    theme_minimal(base_size = 12) +
    theme(plot.title = element_text(color = brand_dark, face = "bold"))

  ggplotly(p)
})
plotlyOutput("scatter_sel", height = "420px")
```

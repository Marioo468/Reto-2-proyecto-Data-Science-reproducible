...
#WAFFLE CHART Valores 3-4 CON HOVER
```{r}
library(dplyr)
library(ggplot2)

# 1) Agregamos ponderando y filtrando NAs
waffle_df <- estudio %>%
  filter(!is.na(gndr), !is.na(w4q50), !is.na(c2weight)) %>%
  mutate(feliz_alta = as.numeric(w4q50 >= 3)) %>%        # 3 y 4 = “A menudo" y "Siempre”
  group_by(gndr) %>%
  summarise(
    total_w = sum(c2weight, na.rm = TRUE),
    alta_w  = sum(c2weight * feliz_alta, na.rm = TRUE),
    prop_alta = alta_w / total_w,
    .groups = "drop"
  ) %>%
  mutate(pct = round(prop_alta * 100))

# 2) Construimos el waffle 10x10 por categoría (Hombre/Mujer) sin tocar el df original
build_waffle <- function(pct, label){
  tibble(
    idx = 1:100,
    fill = idx <= pct,
    gndr = label
...
waffle_cells <- purrr::map2_dfr(waffle_df$pct, waffle_df$gndr, build_waffle) %>%
  group_by(gndr) %>%
  mutate(row = ((idx - 1) %/% 10) + 1,
         col = ((idx - 1) %% 10) + 1) %>%
  ungroup()

# 3) Graficamos etiquetando en el plot (sin mutar el dataset base)
ggplot(
  waffle_cells,
  aes(x = col, y = row, fill = fill)
) +
  geom_tile(color = "white", linewidth = 0.6) +
  scale_y_reverse() +
  scale_x_continuous(expand = c(0,0)) +
  scale_fill_manual(values = c(`TRUE` = "#66AAFF", `FALSE` = "#B3D4FF"), guide = "none") +
  coord_equal() +
  facet_wrap(~ factor(gndr, levels = c(1,2), labels = c("Hombres","Mujeres")), nrow = 1) +
  labs(
    title = "Comparativa felicidad alta (ponderado) por género",
    subtitle = "≥ “A menudo” (niveles 3–4)",
    x = NULL, y = NULL
  ) +
...
#WAFFLE CHART Valores completos
```{r}
library(dplyr)
library(tidyr)
library(ggplot2)
